...
def test_argmax():
    ...
    class Max:
        @R.function
        def main(x: R.Tensor((2, 3, 4, 5), "float32")) -> R.Tensor((2, 4, 5), "float32"):
            gv: R.Tensor((2, 4, 5), "float32") = R.argmax(x, axis=1)
            return gv

    @tvm.script.ir_module
    class Expected:
        @R.function
        def main(x: R.Tensor((2, 3, 4, 5), dtype="float32")) -> R.Tensor((2, 4, 5), dtype="float32"):
...
def test_argmax_symbolic():
    ...
    class Max:
        @R.function
        def main(x: R.Tensor(("a", "b", "c", "d"), "float32")) -> R.Tensor(("a", 1, "c", "d"), "float32"):
            a = T.var("int64")
            c = T.var("int64")
            d = T.var("int64")
            gv: R.Tensor((a, 1, c, d), "float32") = R.argmax(x, axis=1, keepdims=True)
            return gv

    @tvm.script.ir_module
    class Expected:
        @R.function
        def main(x: R.Tensor(("a", "b", "c", "d"), dtype="float32")) -> R.Tensor(("a", 1, "c", "d"), dtype="float32"):
            a = T.Var("a", "int64")
            c = T.Var("c", "int64")
            d = T.Var("d", "int64")
